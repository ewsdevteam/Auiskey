...
class Cart
{
    ...
    {   
        ...
        if ($product->status === 0) {
         ...
         ///============Custom Coded===============/// 
            // For Managing the single item
            $this->cartItemRepository->where(['cart_id' => $cart->id])->delete();
            ///============Custom Coded===============///

        // Cart quantity modified in prepareForCart() in AbstractType.php
    ...
    // Clone function of toArray() for Bid
    public function toArrayBid($cart)
    {
        // $cart = $this->getCart();
    ...
    //Clone function of prepareDataForOrder() for bid
    public function prepareDataForOrderBid($cart): array
    {
        $data = $this->toArrayBid($cart);
        $customer = $this->customerRepository->find($cart->customer_id);
        $finalData = [
            'cart_id'               => $cart->id,
            'customer_id'           => $data['customer_id'],
            'is_guest'              => $data['is_guest'],
